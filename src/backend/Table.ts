...
class Table<M extends {[key: string]: any}> {
    ...
    async insert<K extends keyof M>(model: M, returning?: K): Promise<any | undefined> {
    ...
    async update(id: number | string, model: M) {
    ...
    async query<K extends keyof M, L extends number | undefined>(statements: string[], limit?: L, fields?: K[], order?: string): Promise<L extends undefined ? Pick<M, K>[] : L extends 1 ? Pick<M, K> : Pick<M, K>[]> {
        ...
        if (limit === 1 && result.rows!.length > 0) {
            return result.rows![0] as L extends undefined ? Pick<M, K>[] : L extends 1 ? Pick<M, K> : Pick<M, K>[];
        }

        return result.rows! as L extends undefined ? Pick<M, K>[] : L extends 1 ? Pick<M, K> : Pick<M, K>[];
    }
